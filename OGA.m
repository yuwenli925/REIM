...
hd = 5e-5; t = (hd:5e-5:5)'; t = t.^2;% discretization of the dictionary
nd = length(t);
...
res = C./normg(id(1:i)); % residues of the approximant 
pol = t(id(1:i)); % (-1)*poles of the approximant
maxnode = linspace(1e-6,1,5e5)';
rf = zeros(length(maxnode),1);
for j=1:i
    rf = rf + res(j)./(maxnode+pol(j));
end
err(i) = max(abs(rf - f(maxnode))); % max norm error on [1e-6,1]
end
%--------------------------------------------------------------------------
